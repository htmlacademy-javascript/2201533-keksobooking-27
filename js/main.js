...
let floorFraction = (val, precision)=>
  Math.floor(val * (10 ** precision)) / (10 ** precision);

let ceilFraction = (val, precision)=>
  Math.ceil(val * (10 ** precision)) / (10 ** precision);

let roundFraction = (val, precision)=>
  Math.round(val * (10 ** precision)) / (10 ** precision);

let randomFloat = function(start, end, precision){
  ...
  //return roundFraction((max-min) * Math.random(), precision) + min;
};
